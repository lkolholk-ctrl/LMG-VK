...
- Музыкальные данные, поиск, ссылки, поток и плейлисты — только VK.
- Из других приложений допускается переносить UI, но не API-клиенты, resolver/fallback, DTO или бизнес-логику.
- `WaveHomeScreen` — собственная полноэкранная Aura для персонального VK Mix: без каталожных карточек; VK отвечает за источник и продолжение очереди. Каталожная выдача остаётся в отдельной вкладке New.
