...
- Текущий трек выделяется небольшим accent-контуром/цветом заголовка, скачанный
  ...
  используется только для малых accent-state; серые поверхности используют тот
  же `LiquidSurfaces.card(...)`, что корневые Settings. Быстрые разделы —
  круглые icon-buttons; Sync и иконки Play/Shuffle/Sort тоже круглые, а Search
  остаётся единственной функциональной капсулой в этой управляющей зоне. Тот же
  neutral/circle baseline применён к Playlists, отдельному маршруту Downloads и
  Local Library (Albums/Artists); их поисковые поля также имеют `CircleShape`.
...
## 0.9. Пагинация блоков музыкального каталога

- Подтверждённый в официальном VK 8.185 метод `catalog.getBlockItems` больше не
  подменяется вызовом `catalog.getSection`: запрос отправляет `block_id`,
  `start_from` и optional `ref`, а следующий курсор берётся из
  `response.block.next_from`.
- New не выкачивает все страницы каждого блока во время первоначальной загрузки.
  Первая порция отображается сразу, продолжение добавляется в sheet по мере
  прокрутки; старые элементы не перезаписываются.
- Ошибка следующей страницы больше не превращается в ложный конец списка: она
  доходит до `HomeViewModel` и показывает существующий Retry. Пустая/повторная
  порция с новым курсором не завершает выдачу преждевременно.
- Принудительное обновление New очищает догруженные страницы и использованные
  курсоры старого ответа, поэтому повторно выданный `block.id` не продолжает
  загрузку по устаревшему `next_from`.
- Дедупликация каталога выполняется внутри одного блока. Одинаковый релиз может
  оставаться в нескольких тематических блоках, как его расположил сервер VK.
- `ref` блока сохраняется вместе с `nextFrom` в `HomeCacheManager`. Изменения не
  компилировались; выполнены только лёгкие статические проверки.
