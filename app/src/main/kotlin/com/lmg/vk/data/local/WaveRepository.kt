...
import com.lmg.vk.data.wave.WaveSessionState
...
class WaveRepository(context: Context) {
    ...
    // backend-волна Apple-only; у аккаунтов без apple-сидов ВСЕ её эндпоинты отдают
    ...
    companion object {
        ...
        /** Сколько держим вердикт «backend-волна пуста», не пробивая её заново. */
     ...
     * классифицированные («в начале волны не те треки»). /wave/genre backend починил
     ...
     *
     * Зачем: personal-волна backend для этого аккаунта пуста (она Apple-only, а
     ...
     * (Историческая причина — /wave/genre|mood ВИСЛИ — backend починил на сервере
    ...
    private suspend fun loopPersonalWavePool(
        ...
        val oneShot = MusicBackend.nextBatch(
            limit = requestLimit,
    ...
    suspend fun startPersonalWave(count: Int = FAST_START_COUNT): List<Track> = withContext(Dispatchers.IO) {
        // Короткое замыкание: backend-волну недавно подтвердили пустой — не пробиваем
        ...
        if (legacy.isEmpty()) {
            // Все пути backend-волны пусты → фиксируем вердикт на TTL, чтобы следующие
            // тапы «Моей волны» шли сразу в поиск без повторного backend-шторма.
    ...
    private suspend fun acceptPersonalWaveTracks(
        rawTracks: List<com.lmg.vk.engine.backend.WaveTrack>,
    ...
    private suspend fun fetchLegacyPersonalWave(count: Int): List<Track> {
        ...
        try {
            val first = MusicBackend.startWave().getOrNull()
            if (first?.status == "ok") {
                first.track?.let {
    ...
    private suspend fun fetchGenreOrMoodWaveBatch(
        ...
        val responseResult = when (mode) {
            is WaveMode.Genre -> MusicBackend.genreBatch(
                genre = term,
            ...
            is WaveMode.Mood -> MusicBackend.moodBatch(
                mood = term,
                limit = (targetCount * 2).coerceIn(1, 50),
                diversity = mode.diversity,
                source = mode.source,
    ...
    private suspend fun requestPersonalSessionBatch(
        ...
        return MusicBackend.nextSessionBatch(
            sessionId = sessionId,
            limit = requestLimit,
            diversity = PERSONAL_WAVE_DIVERSITY,
    ...
    private fun shouldRecreateWaveSession(error: Throwable?): Boolean {
        val apiError = error as? BackendException ?: return false
