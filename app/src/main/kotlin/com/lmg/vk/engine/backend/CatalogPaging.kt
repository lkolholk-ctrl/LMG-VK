 ...
 *
 * `exhausted` отдельно от `nextFrom == null`, чтобы зафиксировать подтверждённый
 * конец цепочки курсоров. Пустая или полностью повторная страница сама по себе
 * не является концом, если VK одновременно прислал новый `next_from`.
 */
