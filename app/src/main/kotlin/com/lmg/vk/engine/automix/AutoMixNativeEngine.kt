...
object AutoMixNativeEngine {
    ...
    fun init(context: Context): Boolean {
        ...
        val savedMode = com.lmg.vk.engine.AppSettings.audioCompatMode.value
        runCatching {
        ...
        if (ok) {
            runCatching { com.lmg.vk.engine.AudioFxController.applyToEngine() }
            runCatching { com.lmg.vk.engine.AudioRouteMonitor.reapplyToEngine() }
            ...
            runCatching { com.lmg.vk.debug.DebugLog.add("QUIRKS ${AudioQuirks.describe()}") }
            runCatching { com.lmg.vk.debug.DebugLog.add("OBOE ${audioDiagnostics()}") }
        }
        return ok
    }
